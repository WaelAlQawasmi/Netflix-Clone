...
import UpdateModal from '../FavList/UpdateModal';
...
function Fav(){
    ...
    const getFavRecipes = async () => {
        ...
        return await axios.get(`https://movie-wael.herokuapp.com/getMovies`)
            .then(result => {
    ...
    const deleteFromFav = async(id) =>{
        await axios.delete(`https://movie-wael.herokuapp.com/deleteFavRecipe/${id}`)
                   .then(()=>{
                       console.log("deleted :(")
                      getFavRecipes();
                   }).catch((err)=>{
        ...
        <Row md={3}>
            {
                data.length && data.map((ele) => (
                    <Col key={ele.id} md={40}>
                        <Card className='div-card'>
                            <Card.Img className='div-card-img' variant="top" src={ele.comment} />
                            <Card.Body>
